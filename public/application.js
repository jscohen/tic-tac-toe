...
/***/ function(module, exports, __webpack_require__) {
	...
	var playTurn = function playTurn() {
	  ...
	  var gameArr = games.game.game.cells;
	  var cheatMode = $('.cheatMode').text();
	  ...
	  if (games.gameStarted && turn % 2 === 1) {
	    ...
	    var win = didYouWin(gameArr, X, p1Color);
	    ...
	    var _win = didYouWin(gameArr, O, p2Color);
	    didYouWin(gameArr, O, p2Color);
	    updateGame(cell, O, _win);
	  }
	...
	var whoseTurn = function whoseTurn(counter) {
	  counter = counter + 1;
	  return counter;
	};
	...
	var updateGame = function updateGame(cell, letter, win) {
	  ...
	  api.updateGame(gameUpdate).then(ui.updateGameSuccess).catch(ui.updateGameFail);
	};

	// This function is called from the playTurn function and does the mechanics of a turn
	var doTurn = function doTurn(cell, value, gameArr, turn, letter) {
	  ...
	  var array = addCell(cell, gameArr, letter);
	...
	var getCellNum = function getCellNum(cell) {
	...
	var addCell = function addCell(cellNum, array, letter) {
	  array[cellNum] = letter;
	  return array;
	};

	// Styles the played cell according to the letter
	var styleCell = function styleCell(cell, letter) {
	  if (letter === 'X') {
	...
	var didYouWin = function didYouWin(gameArr, letter, style) {
	...
	// Checks for a draw
	var checkForCatsGame = function checkForCatsGame(letter, style, gameArr) {
	...
	var gameover = function gameover(letter, style) {
	  if (letter === 'X') {
	...
	var activateCheatMode = function activateCheatMode(event) {
	  event.preventDefault();
	...
	var checkCheatMode = function checkCheatMode(gameArr, letter) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	/* WEBPACK VAR INJECTION */(function(module) {// See the players.js documentation
...
/***/ function(module, exports, __webpack_require__) {
	...
	var updateGameSuccess = function updateGameSuccess(game) {
	  return true;
	};
